# L'enregistrement audio portatif

![](https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSrKWuVwsjP4knzmocNWCU5SXMTEE7QzdpVSw&s){data-zoom-image}

## Objectifs

À la fin de ce cours, vous serez capable de :

- Utiliser un enregistreur Sound Devices
- Configurer correctement un enregistrement
- Choisir un microphone adapté à une situation
- Effectuer une prise de son de qualité
- Organiser vos fichiers audio
- Respecter les bonnes pratiques de sécurité auditive



## L'enregistreur Sound Devices

![](https://digitalomnium.com/wp-content/uploads/2019/07/IMG_3148-e1564346414557.png){data-zoom-image}

Le Sound Devices est un enregistreur audio professionnel utilisé pour capturer des sons sur le terrain ou en studio.

### Vérifications avant l'enregistrement

- Carte mémoire installée
- Carte formatée
- Batterie chargée
- Microphone branché
- Casque branché
- Niveau d'entrée vérifié



## Fonction pré-enregistrement

Le pré-enregistrement permet de conserver quelques secondes avant l'appui sur le bouton REC.

### Avantages

- Capture les sons imprévus
- Évite de manquer le début d'une action
- Très utile en documentaire et en prise de son ambiante



## Les microphones

### Microphone dynamique

![](https://upload.wikimedia.org/wikipedia/commons/f/f3/Shure_SM57_--_2023_--_0031-5.jpg){data-zoom-image}

#### Caractéristiques

- Robuste
- Peu sensible aux bruits ambiants
- Adapté aux environnements bruyants

#### Exemples d'utilisation

- Entrevues
- Voix
- Spectacles



### Microphone à condensateur

![](https://img.vistek.net/prodimg/large/423957.jpg){data-zoom-image}

#### Caractéristiques

- Très sensible
- Grande qualité sonore
- Capture beaucoup de détails

#### Exemples d'utilisation

- Ambiances
- Instruments acoustiques
- Studio



### Microphone directionnel

#### Caractéristiques

- Capte principalement devant lui
- Réduit les sons provenant des côtés

#### Exemples d'utilisation

- Tournage vidéo
- Entrevues
- Captation ciblée

## Les diagrammes polaires des autres microphones

![](https://cdn.prod.website-files.com/655e0fa544c67c1ee5ce01c7/655e0fa544c67c1ee5ce11aa_an-artists-guide-to-microphone-polar-patterns-chart.jpg){data-zoom-image}


## Les niveaux audio

![](https://www.tutoriels-mao.com/images/dbFS.gif){data-zoom-image}

### Niveau optimal

Un signal doit être :

✅ Suffisamment fort

❌ Sans atteindre la saturation (clipping)



### Rapport signal/bruit

Le signal représente le son désiré.

Le bruit représente les sons indésirables.

Plus le rapport signal/bruit est élevé, meilleure est la qualité de l'enregistrement.



## Risques auditifs

![](https://www.sciencesetavenir.fr/assets/referentiel/file/14943121.jpg){data-zoom-image}

L'audition est un outil essentiel en conception sonore.

#### Bonnes pratiques

- Éviter les volumes excessifs
- Limiter les longues périodes d'écoute forte
- Prendre des pauses régulièrement
- Retirer les écouteurs lors des déplacements



## Créer un studio maison

Pour améliorer la qualité d'un enregistrement :

- Choisir un endroit calme
- Fermer les fenêtres
- Réduire les sources de bruit
- Utiliser des matériaux absorbants (rideaux, couvertures, coussins)



## Gestion des fichiers audio

Une bonne organisation facilite le montage et la recherche des sons.

### Exemple de nomenclature

```text
TP3_AmbianceCorridor_01.wav
TP3_Entrevue_Marie_01.wav
TP3_Cafeteria_02.wav
```

### Bonnes pratiques

- Utiliser des noms explicites
- Ajouter un numéro de prise
- Éviter les espaces et caractères spéciaux
- Classer les fichiers dans des dossiers logiques



## Rapport de prise de son

Pour chaque enregistrement, noter :

- Date
- Lieu
- Type de microphone utilisé
- Description du son
- Conditions particulières



## Emprunt du matériel

### Responsabilités de l'étudiant

- Vérifier le contenu du sac
- Manipuler le matériel avec soin
- Signaler tout problème
- Respecter les dates de retour



## TP3 – Prise de son sur le terrain

Vous devrez enregistrer différents types de sons.

### À l'intérieur

- Corridors
- Escaliers
- Cafétéria
- Portes

### À l'extérieur

- Circulation
- Nature
- Ambiances urbaines

### En studio

- Voix
- Objets
- Bruitages



## À retenir

Un bon enregistrement commence par une bonne écoute.

Le choix du microphone est aussi important que la qualité de l'enregistreur.

Une bonne organisation des fichiers fait partie du travail du concepteur sonore.